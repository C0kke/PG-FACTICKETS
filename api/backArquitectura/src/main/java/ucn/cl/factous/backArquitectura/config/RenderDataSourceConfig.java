...
public class RenderDataSourceConfig {
    ...
    public DataSource dataSource() {
        ...
        // SSL solo para Render (BD remota). Desactivado para hosts locales
        boolean isLocal = urlInfo.host.equals("localhost")
                || urlInfo.host.equals("127.0.0.1")
                || urlInfo.host.equals("postgres");
        if (isLocal) {
            config.addDataSourceProperty("ssl", "false");
            config.addDataSourceProperty("sslmode", "disable");
            System.out.println("🔓 SSL desactivado (host local: " + urlInfo.host + ")");
        } else {
            config.addDataSourceProperty("ssl", "true");
            config.addDataSourceProperty("sslmode", "require");
            System.out.println("🔒 SSL activado (host remoto: " + urlInfo.host + ")");
        }
    ...
    private DatabaseUrlInfo parseRenderDatabaseUrl(String renderUrl) {
        ...
        try {
            ...
            return new DatabaseUrlInfo(jdbcUrl, username, password, database, host);
    ...
    private static class DatabaseUrlInfo {
        ...
        DatabaseUrlInfo(String jdbcUrl, String username, String password, String database, String host) {
